...
with open(csv_file_path, 'r') as file:
    ...
    for row in csv_reader:
        ...
        record = {
            ...
            'password_hash': '',
            'password_salthash': pyodbc.Binary(salt_hash_bytes)
        }
        print(f"user_name: |{user_name.lstrip()}|, password: |{password.lstrip()}|, \
              salt_hash_bytes: {salt_hash_bytes}, {type(salt_hash_bytes)}" )
        query = """
                    INSERT INTO [gpt].[credentials]
                    ( [username], [password_hash], [password_salthash])
                    VALUES (?, ?, ?)
                """
